const config = {
    ...
    sitecoreApiHost: process.env.SITECORE_API_HOST ?? 'https://xmcloud.localhost',
    sitecoreEdgeUrl: process.env.SITECORE_EDGE_URL ?? 'https://xmcloud.localhost',
    sitecoreEdgeContextId: process.env.SITECORE_EDGE_CONTEXT ?? '',
    sitecoreSiteName: process.env.SITECORE_SITE_NAME ?? 'website',
    publicUrl: process.env_PUBLIC_URL ?? 'http://localhost:5000/',
